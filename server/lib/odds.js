// Mirror of client/src/lib/odds.js — kept in sync manually so the server can
// snapshot moneyline odds at the moment a bet is placed.
...
export function computeOdds(match, { preMatch = false } = {}) {
  ...
  const hcpDiff = hcpB - hcpA;

  const aFraction = clamp(0.5 + 0.05 * momentum + 0.022 * hcpDiff, 0.22, 0.78);
  ...
  const SHRINK = 0.6;
  const pAWin = SHRINK * pAWinRaw + (1 - SHRINK) * 0.4;
  const pBWin = SHRINK * pBWinRaw + (1 - SHRINK) * 0.4;
  const pHalve = SHRINK * pHalveRaw + (1 - SHRINK) * 0.2;
...
export function payoutFor(moneyline) {
  // Treat each pick as a $100 stake; return P&L in points if correct.
  if (moneyline == null) return 0;
  if (moneyline > 0) return moneyline;
  return Math.round((100 / Math.abs(moneyline)) * 100);
}
